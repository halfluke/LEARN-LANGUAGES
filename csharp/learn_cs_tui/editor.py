"""External editor ($EDITOR) for Program.cs buffer."""
...
def launch_editor(initial_code: str, *, filename: str = "Program.cs") -> str:
    cmd = resolve_editor_command()
    nanos = time.time_ns()
    tmp_dir = Path(tempfile.gettempdir()) / f"learn-csharp-editor-{nanos}"

"""Textual TUI: chapter list, theory, exercises, code buffer, run (dotnet), hints."""
...
from learn_cs_tui.chapters import Chapter, Exercise, load_chapters
from learn_cs_tui.editor import launch_editor
from learn_cs_tui.executor import ExecutionResult, execute_code
from learn_cs_tui.progress import ProgressStore
from learn_cs_tui.validator import Validator, ValidationResult
...
class LearnCSharpApp(App[None]):
    ...
    def __init__(self) -> None:
        ...
        self._dotnet_project: Path | None = None
    ...
    def on_mount(self) -> None:
        ...
        try:
            ...
            return
        self.progress.load()
        self._dotnet_project = Path(tempfile.mkdtemp(prefix="learn-csharp-tui-"))
        self.refresh_body()

    def on_unmount(self) -> None:
        if self._dotnet_project and self._dotnet_project.is_dir():
            shutil.rmtree(self._dotnet_project, ignore_errors=True)
...
Code: [yellow]r[/yellow] run (dotnet) · [yellow]e[/yellow] external editor · [yellow]b[/yellow] back
...
[/list]
    ...
    def action_run_snippet(self) -> None:
        if self.route not in ("code", "result") or self._exercise() is None:
            return
        if self._run_busy or self._dotnet_project is None:
            return
        self._run_busy = True

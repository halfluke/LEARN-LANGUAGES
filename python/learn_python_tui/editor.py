"""External editor ($EDITOR) for solution.py buffer."""
...
def launch_editor(initial_code: str, *, filename: str = "solution.py") -> str:
    cmd = resolve_editor_command()
    nanos = time.time_ns()
    tmp_dir = Path(tempfile.gettempdir()) / f"learn-python-editor-{nanos}"
